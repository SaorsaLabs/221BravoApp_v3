...
<svelte:head>
	<title>Account Search - 221Bravo App</title>
</svelte:head>
...
<LayoutCombine>
	...
	<span slot="body" class="pb-4">
        <div class="container mx-auto px-4 flex flex-wrap min-w-full md:min-w-0">
            <a href="/">Home </a>/ Account Explorer
        </div>
        <!-- First Section -->
            ...
            <div class="w-full">
                ...
                {#if resolved == null}
                    <p class="pb-1">
                        ...
                        {#if displayName != null}{@html "<br>"} <span class="dark:text-primary-500 text-error-600">{displayName}</span>{/if}
                    </p>
                {:else}
