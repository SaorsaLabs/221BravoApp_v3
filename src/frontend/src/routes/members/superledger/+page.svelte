    ...
    async function searchClick(){
        ...
        tableDataFuture = new Promise(async (resolve, reject) => {
            ...
            for(i = 0; i< searchLen; i++){
                if (searchResult[i]?.data.length > 0) {
                    // balances
                    adjBalance = Number(searchResult[i].data[0].overview.balance)/ Math.pow(10, searchResult[i].decimals);
                    accountTotalDollar += adjBalance*searchResult[i].tokenPrice;
                    adjPrice = formatter.format((adjBalance*searchResult[i].tokenPrice));
                    adjPriceRaw = adjBalance*searchResult[i].tokenPrice;
                    // tx counts
...
<svelte:head>
	<title>SuperLedger Search - 221Bravo App</title>
</svelte:head>
...
<LayoutCombine>
	...
	<span slot="body" class="pb-4">
        <div class="container mx-auto px-4 flex flex-wrap min-w-full md:min-w-0">
            <a href="/">Home </a>/ <a href="/members/home">Members</a>/ SuperLedger
        </div>
        <!-- First Section -->
