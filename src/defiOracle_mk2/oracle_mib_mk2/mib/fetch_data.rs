...
use oracle_shared_mk2::utils::nat_to_u128;
            ...
            match base_size {
                Some(size) => {
                    ...
                    if reverse == false {
                        ...
                        // let spread =  bid_ask_u64.0 - bid_ask_u64.1;
                        // let qt_power = 10f64.powi(quote_dec as i32);
                        // let ret_data = ExchangeSnapshot{
                        //     snapshot_time: time_now,
                        //     swap_pair,
                        //     exchange: Marketplace::ICDEX,
                        //     price: 1.0/p,
                        //     // Note - bid/ask (.1 .0) are reversed as well!
                        //     bid: ((qt_power/ bid_ask_u64.1 as f64)*qt_power) as u64, 
                        //     ask: ((qt_power/ bid_ask_u64.0 as f64)*qt_power) as u64,
                        //     spread_pct: (spread as f64 / bid_ask_u64.1 as f64) * 100.0,
                        //     liquidity: (0_u64, 0_u64),
                        // };
                        // return Ok(ret_data);
                        return Err("REVERSE NOT SUPPORTED ATM".to_string());
                    }
                },
                None => {
...
async fn fetch_icpswap_quote(
    ...
    match Principal::from_text(market_canister_id) {
        Ok(pr_id) => { 
            ...
            let bid_base:u128;
            let ask_base:u128;
            // let bid_rev_cross: f64;
            // let power_to_one: f64;
            let base_size = get_trade_size_exs(
                &swap_pair, 
                &swap_type,
                &reverse, 
                ...
                Ok(rv) => {
                    match rv.0 {
                        Some(ret_value) => {
                            ...
                            //let p: f64 = (ask_tuple.1 + bid_tuple.1) / 2_f64;
                            let data = calc_price_bid_ask(
                                base_decimals,
                                quote_decimals,
                                quote_tuple.0,
