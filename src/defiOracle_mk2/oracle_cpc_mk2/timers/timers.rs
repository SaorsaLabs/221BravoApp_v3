...
use oracle_shared_mk2::{utils::{process_mib_quotes}, shared_types::{OverviewV1, InternalRateEntry}};
...
pub async fn schedule_data_processing(){
    ...
    let i2u = RUNTIME_STATE.with(|s|{
        ...
        match s.borrow().data.internal_rates.get_single_rate(&String::from("ICP/USD")) {
            Some(v) => {
                return v;
            },
            None => {
...
pub async fn schedule_stable_quote_fetch(){
    ...
    match xdr_rate {
        Ok(v) => {
            let rate = v.0 as f64 / 10_000_f64; // /10k to get actual cross
               // check if xdr exists in internal rates
            let exists = RUNTIME_STATE.with(|s| 
                s.borrow().data.internal_rates.does_swap_exist(String::from("ICP/XDR"))
            );
            // update existing
            if exists == true {
                RUNTIME_STATE.with(|s|
                    s.borrow_mut().data.internal_rates.update_single_quote(&String::from("ICP/XDR"), rate, time.clone())
                );
             // else add new
             } else { 
                RUNTIME_STATE.with(|s|
    ...
    match usd_rate {
        Ok(v) => {
            ...
            let exists = RUNTIME_STATE.with(|s| 
                s.borrow().data.internal_rates.does_swap_exist(String::from("ICP/USD"))
            );
            // update existing
            if exists == true {
                RUNTIME_STATE.with(|s|
                    s.borrow_mut().data.internal_rates.update_single_quote(&String::from("ICP/USD"), v.0, time.clone())
                );
             // else add new
             } else { 
                RUNTIME_STATE.with(|s|
