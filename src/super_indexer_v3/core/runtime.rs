...
use crate::indexer::custom_types::{IndexerType, ProcessedTX, SmallTX, BlockHolder};
...
pub temp_processed_tx: Vec<ProcessedTX>
}
...
impl Data {
    ...
    pub fn process_temp_ptx_to_latest_blocks(&mut self){
        for tx in self.temp_processed_tx.clone() {
            self.latest_blocks.push_tx(tx)
        }
    }
}