...
#[derive(CandidType, Deserialize, Serialize, StableType, Hash, Eq, PartialEq, Clone, Debug, PartialOrd, Ord)]
...
impl IDKey {
    ...
    pub fn from_string(input: String) -> Option<IDKey>{
        if input.chars().count() > 134 {return None} // len in bytes not chars
